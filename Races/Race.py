...
class Race:
    ...
    feats: list[dict[str, str| int | dict[str, str | list[str] | list[list[str]] | None]]]
    ...
    def compile(self, response: Response) -> None:
        ...
        if self.source == "":
            self.extract_source(source_content, version)

        self.extract_description(source_content, version)

        self.extract_feats(source_content, version)

    def extract_source(self, source_content: Selector, version: str):
        source = ""
    ...
    def extract_description(self, source_content: Selector, version: str):
        description: list[str] = [""]

        if version == "1.01.141118":
            description_elements = elements_between_elements(
                source_content,
                source_content.css("#toc0")[0],
    ...
    def extract_feats(self, source_content: Selector, version: str):
        feats: list[dict[str, str| int | dict[str, str | list[str] | list[list[str]] | None]]] = []
                        ...
                        for sub_feat in sub_feats:
                            ...
                            if sub_feat.css("ul").get() != None:
                                for subber_feat in sub_feat.css("ul li"):
                                    subber_feat_name = subber_feat.css("strong::text").get()
                                    subber_feat_description = subber_feat.xpath("./text()").get()
                                    
                                    if subber_feat_name != None and subber_feat_description != None:
                                        feats.append({
    ...
    def get_as_dict(self) -> dict[str, str | list[str] | list[dict[str, str| int | dict[str, str | list[str] | list[list[str]] | None]]]]:
        return {
            "name": self.name,
            "link": self.link,
            "source": self.source,
