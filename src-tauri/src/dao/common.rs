...
use crate::serde_utils::{
	serde_as_array, serde_as_array_mapping, serde_as_bool, serde_as_i64, serde_as_object, serde_as_object_from_option, serde_as_string,
	serde_as_u64,
};
	...
	pub fn new(object: Option<&Value>) -> ChooseAbility {
		...
		if object.is_some() {
			if object.unwrap().is_object()
				&& object.unwrap().as_object().unwrap().get("from").is_some()
				&& object.unwrap().as_object().unwrap().get("count").is_some()
			{
	...
	pub fn new(object: Option<&Value>) -> SkillProficiencies {
		...
		if object.is_some() {
			...
			if object.unwrap().is_object() {
				...
				parsed_object =
					serde_as_array_mapping(object, serde_as_object_from_option, Map::new()).get(0).unwrap_or(&Map::new()).to_owned();
			} else {
...
impl LanguageProficiencies {
	pub fn new(object: Option<&Value>) -> LanguageProficiencies {
		...
		if object.is_some() {
			...
			if object.unwrap().is_object() {
				...
				parsed_object = object
					...
					.unwrap()
					.get(0)
					.unwrap_or(&to_value(Map::new()).unwrap())
					.as_object()
					.unwrap_or(&Map::new())
					.to_owned();
			} else {
		...
		return LanguageProficiencies { languages, any_standard, choose_languages, choose_count };
	}
}
		...
		for item in array {
			if item.is_object() {
				for (key, value) in item.as_object().unwrap() {
					if key == "choose" {
						let choose = value.as_object().unwrap();
						choose_filter = choose
							.get("fromFilter")
							.unwrap_or(&to_value(&choose_filter).unwrap())
							.as_str()
							.unwrap_or(&choose_filter)
							.to_string();
...
impl AdditionalSpell {
	fn parse_array_of_string_or_filter_object(
		value: &Value,
		aquired_at: &String,
		recharge: Recharge,
		category: AdditionalSpellCategory,
				...
				for (key, value) in object {
					if vec!["rest", "daily", "resource", "limited"].contains(&key.as_str()) {
						...
						for (_, value) in serde_as_object(&value, Map::new()) {
							additional_spells.extend(AdditionalSpell::parse_array_of_string_or_filter_object(
								&value,
								&aquired_at,
								recharge.clone(),
								AdditionalSpellCategory::NEITHER,
							));
						}
					} else {
						additional_spells.extend(AdditionalSpell::parse_array_of_string_or_filter_object(
							...
							&aquired_at,
							Recharge::NEITHER,
							AdditionalSpellCategory::new(&key),
						));
					}
				}
			} else {
...
impl PackItem {
	pub fn new(item: &Value) -> PackItem {
		...
		return PackItem { name, quantity: object.get("quantity").unwrap_or(&to_value(1).unwrap()).as_u64().unwrap_or(1), special: false };
	}
}
...
impl ValidContainerSlotItem {
	pub fn new(name: String, quantity: Value) -> ValidContainerSlotItem {
		return ValidContainerSlotItem { name: name.to_string(), quantity: quantity.as_u64().unwrap_or(1) };
	}
}
...
impl StartingItem {
	pub fn new(item: Value) -> StartingItem {
		...
		return StartingItem { name, quantity, value, contains_value, display_name, is_pouch, is_special, is_equipment_type, is_money };
	}
}
...
impl Prerequisite {
	pub fn new(list: Option<&Value>) -> Prerequisite {
		...
		return Prerequisite { campaign_requirement, requires_campaign, class_requirement, requires_class };
	}
}
...
impl SkillToolLanguageChoice {
	pub fn new(object: &Value) -> SkillToolLanguageChoice {
		...
		if object.as_object().is_some() {
			...
			language_amount =
				parsed_object.get("anyLanguage").unwrap_or(&to_value(language_amount).unwrap()).as_u64().unwrap_or(language_amount);
...
impl SavingThrowProficiencies {
	pub fn new(object: Option<&Value>) -> SavingThrowProficiencies {
		...
		if object.is_some() && object.unwrap().as_object().is_some() {
			let parsed_object = object.unwrap().as_object().unwrap();
			options =
				parsed_object.get("from").unwrap().as_array().unwrap().iter().map(|f| f.as_str().unwrap_or("N/A").to_string()).collect();
...
impl Entry {
	...
	pub fn default() -> Entry {
		return Entry { name: "N/A".to_string(), entry_type: "N/A".to_string(), content: vec![] };
	}
}
...
impl Die {
	pub fn new_from_hit_die(value: Option<&Value>) -> Die {
		...
		return Die { number: serde_as_u64(object.get("number"), 0), faces: serde_as_u64(object.get("faces"), 0) };
	}
}
...
impl ClassTableGroup {
	pub fn new(value: &Value) -> ClassTableGroup {
		...
		return ClassTableGroup {
			...
			rows_spell_progress: serde_as_array(object.get("rowsSpellProgression"))
				.iter()
				.map(|i| serde_as_array_mapping(Some(i), serde_as_u64, 0))
				.collect(),
		};
	}
}
