...
use super::common::{
	form_key, Ability, AdditionalFeats, AdditionalSpells, Age, ArmorProficiencies, Entry, File, HeightAndWeight, LanguageProficiencies,
	Resist, SkillProficiencies, SkillToolLanguageProficiencies, ToolProficiencies, WeaponProficiencies,
};
...
impl RaceCopy {
	pub fn new(copy: Option<&Value>) -> RaceCopy {
		if copy.is_none() {
			return RaceCopy { name: "N/A".to_string(), source: "N/A".to_string(), modifying: vec![], preserving: vec![] };
		}
...
impl Race {
	pub fn new(object: Value, subraces: Vec<String>) -> Race {
		...
		for other_source in
			p_object.get("otherSources").unwrap_or(&to_value::<Vec<Value>>(vec![]).unwrap()).as_array().unwrap_or(&vec![]).to_owned().iter()
		{
		...
		return Race {
			...
			creature_types: p_object
				...
				.map(|f| f.as_str().unwrap_or("N/A").to_string())
				.collect(),
			creature_type_tags: p_object
				.get("creatureTypeTags")
				.unwrap_or(&to_value::<Vec<Value>>(vec![]).unwrap())
				.as_array()
				.unwrap_or(&vec![])
				.iter()
