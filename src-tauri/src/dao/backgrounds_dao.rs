...
use super::common::{
	form_key, AdditionalFeats, AdditionalSpells, Entry, LanguageProficiencies, Prerequisite, SkillProficiencies,
	SkillToolLanguageProficiencies, Source, StartingEquipment, ToolProficiencies,
};
...
impl Background {
	pub fn new(object: Value) -> Background {
		...
		return Background {
			...
			additional_sources: serde_as_array_mapping(p_object.get("additionalSources"), serde_as_object_from_option, Map::new())
				.iter()
			...
			other_sources: serde_as_array_mapping(p_object.get("otherSources"), serde_as_object_from_option, Map::new())
				.iter()
				.map(|i| Source::new(i.get("source"), i.get("page")))
				.collect(),
		};
