...
from src.DAO.MiscClasses import SkillProficiencies, LanguageProficiencies, Source, ToolProficiencies, AdditionalSpells
...
class Background:
    ...
    def __init__(self, object: dict[str, Any]) -> None:
        ...
        self.skill_proficiencies = SkillProficiencies(['skillProficiencies']) if 'skillProficiencies' in object.keys() else SkillProficiencies(None)
        self.language_proficiencies = LanguageProficiencies(object['languageProficiencies']) if 'languageProficiencies' in object.keys() else LanguageProficiencies(None)
        self.starting_equipment = StartingEquipment(object['startingEquipment']) if 'startingEquipment' in object.keys() else StartingEquipment(None)
        ...
        self.tool_proficiencies = ToolProficiencies(object['toolProficiencies']) if 'toolProficiencies' in object.keys() else ToolProficiencies(None)
        self.feats = object['feats'].keys() if 'feats' in object.keys() else []
        self.from_feature = object['fromFeature'].keys() if 'fromFeature' in object.keys() else []
        self.has_fluff_images = object['hasFluffImages'] if 'hasFluffImages' in object.keys() else False
        self.additional_spells = AdditionalSpells(object['additionalSpells']) if 'additionalSpells' in object.keys() else AdditionalSpells(None)
        self.additional_sources = [Source(source) for source in object['additionalSources']] if 'additionalSources' in object.keys() else []
        self.prerequisite = Prerequisite(object['prerequisite']) if 'prerequisite' in object.keys() else Prerequisite(None)
        self.skill_tool_language_proficiencies = SkillLanguageToolProficiencies(object['skillToolLanguageProficiencies']) if 'skillToolLanguageProficiencies' in object.keys() else SkillToolLanguageProficiencies(None)
        self.other_sources = [Source(source) for source in object['otherSources']] if 'otherSources' in object.keys() else []
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        copy['starting_equipment'] = self.starting_equipment.__dict__
        ...
        copy['prerequisite'] = self.prerequisite.__dict__
        copy['skill_tool_language_proficiencies'] = self.skill_tool_language_proficiencies.__dict__
