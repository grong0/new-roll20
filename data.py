...
def descFromParagraph(paragraph):
    if paragraph != []:
        paragraphElements = elementsInDiv(paragraph)
        paragraphFirstEleSelector = Selector(text=paragraphElements[0])
        strongElements = elementsInDiv(paragraphFirstEleSelector.css("strong")[0])
        strongFirstEleSelector = Selector(text=strongElements[0])
        paragraphText = paragraph.css("p::text").get()
        strongText = paragraphFirstEleSelector.css("strong::text").get()
        emText = strongFirstEleSelector.css("em::text").get()
        
        if paragraphText != None: return paragraphText
        elif strongText != None: return strongText
        return emText
                        ...
                        for subraceHeader in subraceHeaders:
                            ...
                            if subraceHeader.css("p") != None:
                                if type(subraceHeader.css("p")) == Selector.selectorlist_cls:
                                    # The second paragrapgh description for Draconblood contains 
                                    # "Player's Handbook" which isn't in <em> so it takes priority 
                                    # over the <em> and the rest doesn't get added to the subraceDesc. 
                                    # I need to edit the descFromParagraph() so that it doesn't use 
                                    # the elementsInDiv so that I can take the text inbetween the things.

                                    # Or I just remembered from the old code that I took the number of 
                                    # items that I got and spliced them in because I think 
                                    # .css(x::text).getall() returns all the text with the indents as a list.
