...
impl Race {
    pub fn new(race: Value) -> Race {
        ...
        for reprint in race.get("reprintedAs").unwrap_or(&to_value::<Vec<String>>(vec![]).unwrap()).as_array().unwrap_or(&vec![]) {
            reprinted_as.push(reprint.as_str().unwrap_or("").to_string());
        }

        return Race {
            name: race.get("name").unwrap_or(&to_value("N/A").unwrap()).as_str().unwrap_or("N/A").to_string(),
